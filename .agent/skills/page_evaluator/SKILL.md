...
- **重要: 誤判定防止のための再検証プロトコル (Cross-check Protocol)**:
  ...
  1. **比較表 (Comparison Table)**:
     - `curl -s URL | grep -Ei "Comparison|Compare|Hotel Name|Price Range"` を実行。
     - キーワードが近接して3回以上出現する場合、Grid/Flexによる表が存在すると判定。
  2. **アフィリエイト・マイクロコピー**:
     - `rel="sponsored"` を含むリンクの直前 100文字以内に、絵文字（📅, ✨等）や「Check availability」「Best price」等のベネフィット提示がないか精査。
  3. **社会的証明 (Social Proof)**:
     - `curl -s URL | grep -Ei "Review|Score|Rating|[0-9]\.[0-9]"` を実行。
  4. **価格表示 (Price Range)**:
...
- /home/mune1/dev/ritotabi/ritotabi_analytics/src/evaluations/_registry.json を読み込み、新規評価を追加して更新します。
