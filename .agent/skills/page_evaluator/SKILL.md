...
- **構造化データの全量確認（重要）**: JSON-LDを確認する際は、`grep` 等で一部分だけを見るのではなく、`@graph` 全体を確認して `FAQPage` や `ItemList` などの特定の型が含まれていないか執念深くスキャンして検索してください。
- 公開日がページ内から取得できない場合は、ユーザーに確認するか、本日の日付を暫定的に使用します。

### Step 2: ページ分析
- resources/eval_spec.md および **resources/market_data.md** のルーブリック・統計データに従い、以下の7軸でスコアを算出します。
  1. コンテンツ独自性
  2. 写真・ビジュアル
...
- **重要: 誤判定防止のための再検証プロトコル (Cross-check Protocol)**:
  Markdown解析のみで以下の要素が「見当たらない（false）」と判断された場合、必ず `curl + grep` を用いて RAW HTML レベルでの強制スキャンを実行してください。
  1. **比較表 (Comparison Table)**:
     - `curl -s URL | grep -Ei "Comparison|Compare|Hotel Name|Price Range|ホテル名|カテゴリー|最安値|ぴったりな滞在"` を実行。
     - キーワードが近接して3回以上出現する場合、Grid/Flexによる表が存在すると判定。
  2. **アフィリエイト・マイクロコピー**:
     - `rel="sponsored"` または `rel="noopener sponsored"` を含むリンクの直前 100文字以内に、絵文字（📅, ✨等）や「Check availability」「Best price」「空室状況」等のベネフィット提示がないか精査。
  3. **社会的証明 (Social Proof)**:
     - `curl -s URL | grep -Ei "Review|Score|Rating|クチコミ|スコア|[0-9]\.[0-9]"` を実行。
  4. **価格表示 (Price Range)**:
     - `$` または `¥` 記号に続く数値、あるいは `$$$` 等のティア表示をスキャン。
     - 具体的パターン: `curl -s URL | grep -oE "¥[0-9,]+|円|最安値"`

- **重要: 収益予測（pp, pn, po）の算出**: 
  - `resources/market_data.md` の統計情報（市場規模）と `eval_spec.md` (1-4) の競合難易度係数を組み合わせ、市場シェアに基づいた現実的な数値を算出してください。
  - 公開月を1ヶ月目として**24ヶ月分**の数値を配列で生成してください。
- 重要: 推測表現（「〜と思われる」「〜しそうです」）を一切使用せず、事実（「〜枚の写真がある」「〜の記述がない」）のみを記述してください。
- SEOチェックリスト（6項目）とアフィリエイトチェックリスト（10項目）を個別に評価します。
- **CVR多因子の検出（重要）**: 公開されているアドバイスに基づき、以下の要素が見落とされないよう執念深くスキャンしてください。
  - **価格表示**: 「$100〜」「〇〇円〜」等の具体的な数値。
  - **社会的証明**: OTAのスコア（8.5等）や「Excellent」等の評価文。
  - **モバイルスティッキー**: RAW HTMLのクラス名や構成から、モバイル時に追従するボタンがあるか。
  - **緊急性**: 「セール中」「残りわずか」等の文言。
  - **クロスセル**: レンタカーやツアー予約へのリンク。
...
- **カテゴリ固有チェックリスト（categoryChecklist）**: eval_spec.md のセクション 2c に従い、ページの `type` に応じた構成要素の有無をチェックします。
  - **ホテルページ**: `comparisonTable`, `affiliateMicroCopy` を評価。
  - **ランニングページ**: `courseSpecs`, `runBadge`, および **`runningCvr`**（リンク数・CTA密度・ペルソナ適合性）を評価。
  - 該当しないキーは `null` とします。
...
- **重要: 事実ベースの判定**: FAQの有無は、以下の基準で判定してください。ソースコードを直接見られない場合、推測で `true` とすることを厳禁します。
  - **ランニングページ以外**: Markdownテキスト内にQ&A形式の記述（「Q:」「A:」や質問と回答の対）が物理的に存在する場合に `true` とする。**注意**: QやAがアイコン画像やスタイル付きの装飾である場合、「Q:」「A:」という文字列そのものが存在しない可能性があるため、質問文らしきテキスト（例：「どのホテルがおすすめですか？」等）の有無も併せて確認してください。
  - **ランニングページ**: JSON-LDにFAQPageの定義が存在する場合に `true` とする（情報の重複を避けるため、視覚的なQ&Aセクションは任意とする）。
- **重要: 比較表の判定**: ホテルページ等で必須とされる比較表は、以下の基準で判定してください。
  - `<table>` タグの有無だけでなく、複数のホテルを横断的に比較する項目（例：「HOTEL NAME」「CATEGORY」「PERFECT FOR」「PRICE RANGE」等のヘッダーラベル）が物理的に近接して存在するか確認してください。
  - モダンな実装では `div` や `grid` 等で表形式に見せているケースが多いため、タグ名に依存せず、情報の並びとラベル内容で事実を判定してください。
